...
//Доступ к следующему элементу списка медленее, чем к следующему элементу массива из за того, что массив хранится единым блоком в памяти и кешируется в процессоре,
//а каждая нода связанного списка может хранится в случайном участке памяти, доступ к которой медленее, чем к кешу процессора

//Подводя итог, стоит сказать, что данная реализация лучше с точки зрения менеджмента памяти, но довольно медленная